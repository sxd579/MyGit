    ...
    for start_epoch in range(35):
        for step, (tx, ty) in enumerate(trainloader, 0):
            ...
            if step % 10 == 9:  # 每2000个batch打印一下训练状态
                ...
                print('Epoch:', start_epoch, '|Step:', step, '|train loss:%.4f' % loss.item(), '|test accuracy:%.4f' % acc)
            state = {'model': net.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': start_epoch}
